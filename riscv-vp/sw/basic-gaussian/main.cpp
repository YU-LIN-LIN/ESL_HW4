...
int main(int argc, char *argv[]) {
  ...
  for(int j = 0; j < height; j++){
    for(int i = 0; i < width; i++){ 
      ...
      r_ans = data.uc[0];     // printf("r_ans = %d\n", r_ans);
      g_ans = data.uc[1];
      if (j == 0) {
        printf("r_ans = %d\n", r_ans);
        printf("g_ans = %d\n", g_ans);
      ...
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 2) = r_ans;
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 1) = g_ans;
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 0) = b_ans;
    }
  }
